...
function NdaDevForm({ data, onChange }: FormProps) {
  ...
  const setRoot = (field: keyof Pick<NdaData, 'effectiveDate' | 'city' | 'documentDate' | 'juridiction'>) =>
      ...
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div className="field">
            ...
            <input value={d.juridiction} onChange={setRoot('juridiction')} placeholder="Paris" required />
          </div>
        </div>
      </div>
    </>
...
function NdaDevDocument({ data, party1SignedAt, party2SignedAt }: DocumentProps) {
  const { party1, party2, effectiveDate, city, documentDate, juridiction } = data as unknown as NdaData;
...
export const ndaDev: Template = {
  id: 'nda-dev',
  category: 'Accord de confidentialité',
  label: 'NDA — Développement',
  description: 'Adapté aux missions de développement informatique. Inclut une liste détaillée des informations confidentielles techniques.',
